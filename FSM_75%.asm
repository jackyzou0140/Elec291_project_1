...
; (1) ABORT -> state0 and "NOT STARTED": clears run_flag and on_flag
; (2) LCD state digit synced with HEX5 (fix off-by-one)
; ============================================================
...
; ============================================================
; BUZZER / SPEAKER OUTPUT (sound feedback)
; ============================================================
BUZZER_OUT equ P3.7
...
;
; ---- Buzzer/beeper scheduler (non-blocking, driven by 1ms ISR) ----
beep_timer_ms:  ds 1     ; phase timer in ms
beep_count:     ds 1     ; beeps remaining
beep_pri:       ds 1     ; priority (0 none, 1 state, 2 done, 3 error)
state_sec_lo:   ds 1
state_sec_hi:   ds 1
thr_soak_lo:    ds 1
thr_soak_hi:    ds 1
...
beep_active:     dbit 1
tone_active:     dbit 1
beep_phase_on:   dbit 1     ; 1=ON phase, 0=OFF gap
...
; ============================================================
; BUZZER SUPPORT (non-blocking)
; - One beep on state change
; - Five beeps on process complete
; - Ten beeps on error
; Buzzer output pin: P3.7 (BUZZER_OUT)
; Timing: ON=80ms, OFF=80ms, tone toggles each 1ms (~500Hz)
; ============================================================
BEEP_ON_MS   EQU 80
BEEP_OFF_MS  EQU 80

; Start a beep sequence with priority.
;   A = number of beeps (1..10)
;   R7 = priority (1 state, 2 done, 3 error)
Beep_Start:
    ; If a sequence is already running with >= priority, ignore.
    mov R6, a           ; save requested beep count
    jb  beep_active, BS_chkpri
    sjmp BS_do
BS_chkpri:
    mov a, beep_pri
    clr c
    subb a, R7
    jnc BS_ret          ; existing priority >= new
BS_do:
    ; store new priority
    mov beep_pri, R7
    ; store beep count
    ...
    clr tone_active
    clr beep_phase_on
    mov beep_timer_ms, #1   ; start almost immediately
    clr BUZZER_OUT
BS_ret:
    ret

Beep_Request_StateChange:
    mov a, #1
    mov R7, #1
    sjmp Beep_Start
...
T0_buzzer:
    ; ---- Buzzer scheduler (1ms) ----
    jnb beep_active, T0_buz_idle
    ; phase timer--
    mov a, beep_timer_ms
    jz  T0_buz_phase
    dec beep_timer_ms
T0_buz_phase:
    ...
    jb  beep_phase_on, T0_buz_to_off
    ; OFF -> ON (consume one beep)
    mov a, beep_count
    jz  T0_buz_stop
    dec beep_count
    setb beep_phase_on
    setb tone_active
    mov beep_timer_ms, #BEEP_ON_MS
    clr BUZZER_OUT
    sjmp T0_buz_tone
T0_buz_to_off:
    clr beep_phase_on
    clr tone_active
    mov beep_timer_ms, #BEEP_OFF_MS
    clr BUZZER_OUT
    sjmp T0_buz_tone
T0_buz_stop:
    clr beep_active
...
T0_buz_tone:
    jnb tone_active, T0_buz_idle
    cpl BUZZER_OUT       ; ~500Hz (toggle each 1ms)
T0_buz_idle:
...
GS_DONE2:
    ...
    mov a,R6
    dec a              ; <<< FIX (2): ui_st1->0, ui_st2->1 ... sync with HEX5
    add a,#'0'
    ret
GS_NONE:
    mov a,#'0'
...
; ============================================================
; <<< FIX (1): ABORT helper - go to state0 and "NOT STARTED"
; Clears run_flag so FAST sync won't re-enable on_flag.
; Also sets stop_pulse so UI/logic consistent.
; ============================================================
FSM_AbortToIdle:
    clr run_flag        ; <<< FIX: abort means not started anymore
    setb stop_pulse     ; optional but helpful (tell UI it stopped)
    clr on_flag
    mov fsm_state, #0
    mov pwm_mode, #0
    clr a
...
F_START:
    ...
    lcall Beep_Request_StateChange
    sjmp F_RUN
...
ST1:
    ...
    ; <<< FIX (1): abort to state0 and "NOT STARTED"
    lcall Beep_Request_Error
